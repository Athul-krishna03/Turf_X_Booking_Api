...
export class TurfRepository
  extends BaseRepository<ITurfModel>
  implements ITurfRepository
{
  constructor() {
    super(TurfModel);
  }
  ...
  async findByIdAndUpdateStatus(id: string): Promise<void> {
    ...
    const result = await TurfModel.findByIdAndUpdate(
      { _id: id },
  ...
  async updateProfileById(
    id: string,
    data: Partial<ITurfEntity>
  ): Promise<TurfProfileResponse> {
  ...
  async findByIdAndUpdatePassWord(id: string, password: string): Promise<void> {
    const updatePass = await TurfModel.findByIdAndUpdate(
      { _id: id },
